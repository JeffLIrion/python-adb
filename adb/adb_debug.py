...
def Devices(args):
    ...
    args : argparse.ArgumentParser
        CLI arguments; see :func:`adb.common_cli.GetDeviceArguments`.
    ...
    for d in adb_commands.AdbCommands.Devices():
        if args.output_port_path:
            print('%s\tdevice\t%s' % (d.serial_number, ','.join(str(p) for p in d.port_path)))
        else:
...
def List(device, device_path):
    ...
    for f in files:
        mode = (('d' if stat.S_ISDIR(f.mode) else '-') +
...
def main():
    ...
    common.add_argument('--rsa_key_path', action='append', default=[], metavar='~/.android/adbkey',
                        help='RSA key(s) to use, use multiple times to load mulitple keys')
    common.add_argument('--auth_timeout_s', default=60., metavar='60', type=int,
    ...
    subparser.add_argument('--output_port_path', action='store_true', help='Outputs the port_path alongside the serial')
    ...
    common_cli.MakeSubparser(subparsers, parents, adb_commands.AdbCommands.Push,
                             {'source_file': 'Filename or directory to push to the device.'})
    common_cli.MakeSubparser(subparsers, parents, adb_commands.AdbCommands.Pull,
    ...
    common_cli.MakeSubparser(subparsers, parents, adb_commands.AdbCommands.RebootBootloader)
    common_cli.MakeSubparser(subparsers, parents, adb_commands.AdbCommands.Remount)
